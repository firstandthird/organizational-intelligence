...
/**
 * @template {import('./repository-types.mjs').SharedContextEntry | import('./repository-types.mjs').PromptEntry} T
 * @param {Map<string, T>} byId
 * @param {Map<string, T>} byRelPath
 * @param {string} id
 * @returns {T | null}
 */
function lookupStoreEntry(byId, byRelPath, id) {
  const normalized = normalizeEmbedTarget(id);
  if (normalized) {
...
export class FileMdSharedContextStore {
  ...
  async fetch(id) {
    this._ensureLoaded();
    const entry = lookupStoreEntry(this.byId, this.byRelPath, id);
    if (!entry) {
      return null;
    }
