...
export class FileMdSharedContextStore {
  ...
  async load() {
    ...
    logMarkdownFiles("sharedContext load", this.dir, files);
  ...
  async list(opts) {
    this._ensureLoaded();
    const files = await readdir(this.dir).catch(() => []);
    logMarkdownFiles("sharedContext list", this.dir, files);
...
export class FileMdPromptStore {
  ...
  async load() {
    this.byId.clear();
    const files = await readdir(this.dir).catch(() => []);
    logMarkdownFiles("prompts load", this.dir, files);
    for (const f of files) {
      if (!f.endsWith(".md")) continue;
      const rel = `prompts/${f}`;
      const full = join(this.dir, f);
  ...
  async list(opts) {
    this._ensureLoaded();
    const files = await readdir(this.dir).catch(() => []);
    logMarkdownFiles("prompts list", this.dir, files);
    let entries = [...this.byId.values()].sort((a, b) => a.id.localeCompare(b.id));
