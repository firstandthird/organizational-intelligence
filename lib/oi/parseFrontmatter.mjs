 ...
 * @param {string} key
 */
function fieldStringList(fields, key) {
  const value = fields[key];
...
export function serializePromptMarkdown(entry) {
  ...
  if (audience.length) {
    fm += serializeYamlStringList("audience", audience);
  }
  if (useWhen.length) {
    fm += serializeYamlStringList("use_when", useWhen);
  }
  if (context.length) {
    fm += serializeYamlStringList("context", context);
  }
  if (args.length) {
    fm += serializeYamlStringList("arguments", args);
  }
...
export function promptEntryFromParsed(fields, body, sourceRelPath) {
  ...
  const tools = fieldStringList(fields, "tools");
  const allowedAgents = fieldStringList(fields, "allowed_agents");
  const tags = fieldStringList(fields, "tags");
  ...
  return {
    ...
    tags,
    audience: fieldStringList(fields, "audience"),
    useWhen: fieldStringList(fields, "use_when"),
    context: fieldStringList(fields, "context"),
    arguments: fieldStringList(fields, "arguments"),
    text: body,
    sourceRelPath
  };
}
