...
const sharedContextInputSchema = z.object({
  operation: z.enum(["list", "search", "fetch", "upsert"]),
  cursor: z.string().optional(),
  limit: z.number().int().positive().max(200).optional(),
  tagsAny: z.array(z.string()).optional(),
...
const promptRepositoryInputSchema = z.object({
  operation: z.enum(["list", "search", "fetch", "upsert"]),
  cursor: z.string().optional(),
  limit: z.number().int().positive().max(200).optional(),
  tagsAny: z.array(z.string()).optional(),
...
const subMcpProxyInputSchema = z.object({
  operation: z.enum(["list_servers", "listServers", "list_tools", "listTools", "invoke"]),
...
const sharedContextTool = {
  ...
  schema: sharedContextInputSchema,
  async run(input) {
...
const promptRepositoryTool = {
  ...
  schema: promptRepositoryInputSchema,
  async run(input) {
...
export {
  sharedContextInputSchema,
  sharedContextOperationSchema,
  promptRepositoryInputSchema,
  promptRepositoryOperationSchema,
  subMcpProxyInputSchema,
